...
和老彭讨论下，他认为现有的分割+空洞填补工作量会比较大，建议先对时间域进行工作，本来以为非常简单的背景建模，打算明天建立一个简单的模型，在*VSRS*上实现，实际和李金龙讨论后发现里面的工作量也是非常大，我应先对其进行调研，找了*2017*年三篇虚拟视点绘制和空洞填补的文献，明天看完（有一个大概印象即可）

在我的争取下，老彭同意让我两个工作点同时推进，所以*TensorFlow*的学习仍然可以继续，但是以**时间域**处理工作为重
...
明日工作计划：
  ...
  - 翻译三篇文献
  - 在*VSRS*平台实现背景建模的方法，以最简单的背景构建法来做
  
PS：

今天*GeJN*身体不舒服，晚上好好安慰一下
...
今日任务一样都没有完成，帮助朱雯青做了一个很简单的分水岭算法，感觉不是做自己感兴趣的东西，力量就很很弱

这种状态很不好，你需要好好冷静一下考虑，尽快落实到一个具体工作中来，又一个成型的工作思路，而不是游离在外的浪子

近期目标，整理一个具体的工作思路，*调研->可能的方向->实践探索->成文*

先从调研开始。关于看文献的薄弱，无需每句翻译，看个摘要，直接可以看核心算法了，人生苦短。。。
